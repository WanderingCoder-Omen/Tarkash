...
set_credentials() {
    ...
    echo -n "    Please choose a username for Tarkash's backend: "
    read login
    echo -n "    Please choose a password for Tarkash's backend: "
    read -s password1
    echo ""
...
set_kioskmode() {
    echo -n "[?] Do you want to start Tarkash in fullscreen during the system startup (aka. Kiosk mode)? [Yes/No] "
    read answer
...
set_update() {
    echo -n "[?] Do you want to be able to update Tarkash from the frontend interface? [Yes/No] "
    read answer
    if [[ "$answer" =~ ^([yY][eE][sS]|[yY])$ ]]
    then
...
create_directory() {
    # Create the Tarkash directory and move the whole stuff there.
    echo -e "[+] Creating Tarkash folder under /usr/share/"
...
[Unit]
Description=Tarkash frontend service
...
[Unit]
Description=Tarkash backend service
...
[Unit]
Description=Tarkash watchers service
...
change_hostname() {
   # Changing the hostname to Tarkash
   echo -e "[+] Changing the hostname to Tarkash"
   echo "tarkash" > /etc/hostname
   sed -i "s/$HOST/tarkash/g" /etc/hosts

   # Adding tarkash.local to the /etc/hosts.
   echo "127.0.0.1  tarkash.local" >> /etc/hosts
}
...
Name=Tarkash
Comment=Launcher for the Tarkash frontend
        ...
        if echo "$iface" | grep -Eq "(wlan[0-9]|wl[a-z0-9]{20})"; then
            ...
            if echo "$config" | grep -qv "inet "; then              # Test if not currently connected
                ...
                if echo "$info" | grep -qE "* AP$"; then            # Know if the iface has the AP mode available.
                    echo -n "[?] The interface $iface can be used for the Wi-Fi Access Point. Do you want to use it for the Tarkash Access Point ? [Yes/No] "
                    read answer
                    if [[ "$answer" =~ ^([yY][eE][sS]|[yY])$ ]]
                    then
                        IFACE_IN="$iface"
...
if [[ $EUID -ne 0 ]]; then
    ...
    echo "You have a Tarkash instance already installed on this box."
